...
p <- p + xlab("Genpop - donor median ferritin (ng/l)") + ylab("General population coefficient estimate")
p <- p + scale_colour_continuous(name="Donor\ncohort median\nferritin (ng/l)")
#these need rethinking
#p <- p + geom_text(aes(x = 25.6 - 26,  y =plotdata_wnd$m[6],label="as_pre_fi_pg"),col="red")
#p <- p + geom_text(aes(x = 55,  y =plotdata_wnd$m[13],label="original_gp"),col="red")
#p <- p + geom_point(aes(x = 25.6 - 26,  y =plotdata_wnd$m[6]),col="red")
#p <- p + geom_point(aes(x = 55,  y =plotdata_wnd$m[13]),col="red")
#p <- p + facet_grid(. ~ Cohort)
p
...
tmp <- regression_cohorts %>% filter(Cohort == "FINDONOR") %>% 
  mutate(
    donation_count = donation_count/0.5,
    last_donation = last_donation*0.5
    )
...
plta <- cowplot::plot_grid(plt_1,plt_3, plt_2,plt_4,  ncol = 2,nrow=2)
...
# Generating x values
x <- runif(n, -10, 10)  # Generating random x values between -10 and 10

# Creating an empty plot to add scatter plots with varying slopes
plot(NA, NA, xlim = c(-10, 10), ylim = c(-20, 20), xlab = "X", ylab = "Y", main = "Scatter plots with varying slopes")

# Varying slopes from -1 to 1
for (slope in seq(-1, 1, by = 0.2)) {
  # Computing y values based on the varying slope
  y <- slope * x + rnorm(n, 0, 5)  # Simulating y values with noise
  
  # Plotting scatter plot with varying slopes
  points(x, y, col = rainbow(length(seq(-1, 1, by = 0.2)))[which(seq(-1, 1, by = 0.2) == slope)], pch = 16)
}

# Adding a legend for different slopes
legend("topleft", legend = seq(-1, 1, by = 0.2), col = rainbow(length(seq(-1, 1, by = 0.2))), pch = 16, title = "Slope")
