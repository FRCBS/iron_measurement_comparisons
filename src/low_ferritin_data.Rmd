...
fr1997_summary_final <- fr1997_summary_final %>% 
  ...
  mutate(Group = ordered(Menstruation, levels =  c("regular_period", "irregular_period", "no_period"))) 
...
We remove `r cohort_name %>% filter((group != "Men" & is.na(PreviousChildbirth)))%>% nrow()` female donors who did not answer the question on childbirth: 
  ...
  * FinRisk97: 8 `r fr1997_summary_final %>% filter((Group != "Men" & is.na(PreviousChildbirth)))%>% nrow()`
  * Health2000: 0 `r h2000_summary_final %>% filter((Group != "Men" & is.na(PreviousChildbirth)))%>% nrow()`
...
Only women <45 yrs were asked whether they are currently pregnant and if so, how many weeks (source: https://www.julkari.fi/bitstream/handle/10024/78181/2005b6.pdf?sequence=1&isAllowed=y, page 56). This means a total of 1246 n/a:s for this specific question. We added a filter for age <45 years old, which leaves us with 111 missing answers. NA:s in >45-year-oldes are imputed as not pregnant. Although pregnancy is possible after this age, chances are lower. We used this age as our cut-off for menopause. 
...
We will now remove `r cohort_name %>% filter((Group != "Men" & is.na(CurrentPregnancy)))%>% nrow()` females who did not answer the pregnancy question: 
  ...
  * Health00: 111 `r h2000_summary_final %>% filter((Group != "Men" & Age <=45 & is.na(CurrentPregnancy)))%>% nrow()`
...
We remove `r cohort_name %>% filter(is.na(RedMeat)) %>% nrow()` participants for whom we do not have Smoking data:
  ...
  * FinRisk97: 28 `r fr1997_summary_final %>% filter(is.na(RedMeat)) %>% nrow()`  
...
```
 ...
 Total number of cohort participants removed for missing questionnaire answers

The total number removed because of missing questionnaire data is:
`r nb_removed_blood` = 126
`r nb_removed_fr97` = 63
`r nb_removed_h2000` = 179
...
This amounts to `r cohort_name %>% filter(BMI >= 50 | CRP >= 30 | Ferritin >= 400) %>% nrow()` participants that are removed. 
  ...
  * Health00: 137 `r h2000_summary_final %>% filter(BMI >= 50 | CRP >= 30 | Ferritin >= 400) %>% nrow()`
...
test_data_pre_women <- regression_cohorts %>% 
  select(ID, Smoking, Sex, Ferritin, Hb)  
         #luokkamuuttujat mitä ei transformoida tähän kohtaan
...
test_data_post_women <- regression_cohorts %>% 
  ...
  mutate(Age = scale(Age, scale = FALSE)[,1],
         ...
         BMI = scale(BMI, scale = FALSE)[,1],
         donation_count_2 = donation_count^2) %>% 
  full_join(test_data_post_women, by = "ID")  %>% 
  dplyr::select(-ID)
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "Age.x", "log_CRP.x",  "donation_count.x", "donation_count_2.x",
                    "log_last_donation.x"),
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "PreviousChildbirth",  "RedMeat_n", "Smoking", "GivenIronSupplements", "BMI"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "Age.y"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "log_CRP.x"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "donation_count.x"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "donation_count_2.x"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "log_last_donation.x"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "PreviousChildbirth"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "RedMeat_n"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "Smoking"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
...
ggpairs(test_data_pre_women, 
        columns = c("log_ferritin.x", "GivenIronSupplements"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
```
