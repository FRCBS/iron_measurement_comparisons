...
```{r}
# Assign participants to their specific cohorts 
...
We are missing menstruation data from >37 % of female Health2000 participants. Menstruation was defined as: 
...
Removing n/a:s for this variable `r h2000_summary %>% filter(Sex == "Women" & is.na(MENOP)) %>%  nrow()` causes loss of 62 participants.
...
Of these, `r cohort_summary_name %>% filter(Sex == "Women" & is.na(Menstruation) & age > 55) %>%  nrow()` are older than 55. We impute these to no period so they can be included in the postmenopausal women. 
...
# Add columns for the variables that are missing from the cohorts (CurrentPregnancy for FinDonor, RedMeat for Health2000 and all iron supplement variables for both general population cohorts) and add values to them (NA for history of iron supplements, 0 for the rest)
...
Because 1246 Health2000 cohort participants didn't answer the pregnancy question we will add a filter for <45 years old, which leaves us with 111 missing answers. NA:s in >45-year-oldes are imputed as not pregnant. Although pregnancy is possible after this age, chances are lower. We also used this age as our cut-off for menopause. We will now remove `r cohort_name %>% filter((group != "Men" & is.na(QR83)))%>% nrow()` females who did not answer the pregnancy question: 
