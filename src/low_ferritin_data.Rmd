...
h2000_missing_menstruation <- h2000_mens_missing %>% 
  filter(Age < 55 & !is.na(Menstruation)) 

women_over_55 <- h2000_mens_missing %>% 
  filter(Age >= 55) 

# assign women > 55 yrs to no menstruation
women_over_55$MENSTRUATION <- "no"

# join the two separate tables 

h2000_missing_menstruation <- h2000_missing_menstruation %>% 
  group_by(ID) %>% 
  dplyr::select(MENSTRUATION, Stage, AgeGroup, Age) %>% 
  ungroup()

h2000_missing_menstruation <- h2000_missing_menstruation %>% 
  full_join(women_over_55, by = "ID") 
