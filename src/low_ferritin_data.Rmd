...
There are "cohort_name %>% distinct(ID_variable) %>% nrow()" individials enrolled in the studies. 
  ...
  * FinDonor: 2584   indiv_donations_data %>% distinct(donor) %>% nrow()
  * FinRisk97: 7943  fr1997 %>% distinct(RELEASE_ID) %>% nrow()
  * Health2000: 6264 h2000 %>% distinct(RELEASE_ID) %>% nrow()
...
```{r}
write.table(regression_cohorts, file = paste0("../results/regressioncohorts",gsub("-","",as.Date(Sys.time())),".txt"))
```
...
```{r}
write.table(test_data_pre_women, file = paste0("../results/test_data_pre_women",gsub("-","",as.Date(Sys.time())),".txt"))
```
...
```{r}
write.table(test_data_post_women, file = paste0("../results/test_data_post_women",gsub("-","",as.Date(Sys.time())),".txt"))
```
...
```{r}
write.table(test_data_men, file = paste0("../results/test_data_men",gsub("-","",as.Date(Sys.time())),".txt"))
```
...
ggpairs(test_data_pre_women, ggplot2::aes(colour = Cohort, alpha = 0.5),
        columns = c("log_ferritin", "iron_deficiency", "PreviousChildbirth",  "RedMeat_n", "Smoking", "iron_complience", "BMI"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE) + 
ggtitle("Premenopausal women")
...
ggpairs(test_data_men, ggplot2::aes(colour = Cohort, alpha = 0.5),
        columns = c("log_ferritin", "iron_deficiency",  "RedMeat_n", "Smoking", "iron_complience", "BMI"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE) + 
ggtitle("Men")
...
ggpairs(test_data_pre_women_h2000, 
        columns = c("log_ferritin", "iron_deficiency", "PreviousChildbirth", "Smoking", "iron_complience", "BMI"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
...
ggpairs(test_data_men_h2000, 
        columns = c("log_ferritin", "iron_deficiency", "Smoking", "iron_complience", "BMI"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)
...
```{r}
rmarkdown::render("low_ferritin_data.Rmd")
```
