...
fr1997_summary <- fr1997 %>%
  ...
  group_by(RELEASE_ID) %>%
  dplyr::select(IKA, SUKUP, FERRITIN, HGB, CRP, RELEASE_ID, K129, BMI, KY100_22, Q69, KY163, GRAVID, Cohort, ALUE, PAINO) %>%
  ungroup()
...
h2000$SP2 <- gsub("2", "Women", h2000$SP2)  
 ...
 h2000_summary <- h2000 %>%
    ...
    group_by(RELEASE_ID) %>%
    dplyr::select(IKA2, SP2, FERRITIINI, B_Hb, CRP, RELEASE_ID, BD03, BMII_BMI.x, FB05, synnytys, BD07, Cohort, MENOP, BD06, SHP_2000, BMII_PAINO.x) %>%
    ungroup()  
...
names(h2000_summary)[names(h2000_summary) == "SHP_2000"] <- "Region"
...
We remove "cohort_name %>% filter(is.na(BMI)) %>% nrow()" participants for whom we do not have smoking data:
  ...
  * FinRisk97: `r fr1997_summary_final  %>% filter(is.na(Smoking)) %>% nrow()`
           * `r fr1997 %>%  filter(is.na(Q69)) %>%  nrow()` 43 % missing
  * Health2000: 2153 `r h2000_summary_final %>% filter(is.na(Smoking)) %>% nrow()` 
           * `r h2000 %>%  filter(is.na(FB05)) %>%  nrow()` 44 % missing
...
* BMI > 50
* Ferritin > 400 
* Weight < 50kg (blood donor selection criteria, only removed from general population cohorts) 

This amounts to "cohort_name %>% filter(BMI >= 50 | Ferritin >= 400 | Weight < 50) %>% nrow()" participants that are removed. 
  * FinDonor: `r blood_data_summary_final %>% filter(BMI >= 50 | Ferritin >= 400) %>% nrow()`
...
Assign the participants from the capital region (Helsinki, Espoo and Vantaa) to "Helsinki" and everyone else to "other" under the Region variable. FinDonor study participants all come from the capital region.

Health2000:
	1	Uusimaa <- this one includes the capital region but also all other Uusimaa areas, don't use 
	2	Helsinki <-  only Helsinki, we won't use Espoo and Vantaa for the Health2000 cohort
	3	Varsinais-Suomi
	4	Satakunta
	5	Kanta-Häme
	6	Pirkanmaa
...
FinRisk97:
  ...
  3 North Savonia
  4 Turku and Loimaa
  5 Helsinki and Vantaa <-- use this one, does not include Espoo
  6 Oulu province 

```{r}
blood_data_summary_final$Region <- "HKI"
...
test_data_pre_women <- test_data_pre_women %>% 
  dplyr::select(ID, Group, Sex, Age, donation_count, last_donation, Ferritin, Hb, BMI, Menstruation, Smoking, RedMeat_n, iron_complience, PreviousChildbirth, CRP, Cohort, donation_count_2, log_ferritin, log_last_donation, log_CRP, iron_deficiency, Region) %>%
...
ggpairs(test_data_pre_women, ggplot2::aes(colour = Cohort, alpha = 0.5),
        columns = c("log_ferritin", "iron_deficiency", "PreviousChildbirth",  "RedMeat_n", "Smoking", "iron_complience", "BMI", "Menstruation", "Region"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE) + 
ggtitle("Premenopausal women")
...
ggpairs(test_data_post_women, ggplot2::aes(colour = Cohort, alpha = 0.5), 
        columns = c("log_ferritin", "iron_deficiency", "PreviousChildbirth",  "RedMeat_n", "Smoking", "iron_complience", "BMI", "Region"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)+ 
ggtitle("Postmenopausal women")
...
ggpairs(test_data_men, ggplot2::aes(colour = Cohort, alpha = 0.5),
        columns = c("log_ferritin", "iron_deficiency",  "RedMeat_n", "Smoking", "iron_complience", "BMI", "Region"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE) + 
ggtitle("Men")
...
```{r}
ggpairs(test_data_pre_women, ggplot2::aes(colour = Cohort, alpha = 0.5),
        columns = c("log_ferritin", "iron_deficiency", "Menstruation", "Region"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE) + 
ggtitle("Premenopausal women")
...
ggpairs(test_data_post_women, ggplot2::aes(colour = Cohort, alpha = 0.5), 
        columns = c("log_ferritin", "iron_deficiency", "Region"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE)+ 
ggtitle("Postmenopausal women")
