...
blood_data_summary_final_test <- blood_data_summary_final %>% 
  mutate(Group = case_when(
    Sex == "Men" ~ "Men", 
    Sex == "Women" & (Menstruation == "no_period" & Age > 45) ~ "Post_menopause_women",
    Sex == "Women" & (Menstruation == "no_period" & Age <= 45) ~ "Women_pre_menop_no_mens",
...
This amounts to "cohort_name %>% filter(BMI >= 50 | CRP >= 30 | Ferritin >= 400) %>% nrow()" participants that are removed. 
  * FinDonor: `r blood_data_summary_final %>% filter(BMI >= 50 | Ferritin >= 400) %>% nrow()`
  * FinRisk97: `r fr1997_summary_final %>% filter(BMI >= 50 | Ferritin >= 400) %>% nrow()`
  * Health00: `r h2000_summary_final %>% filter(BMI >= 50 | Ferritin >= 400) %>% nrow()`
...
#Health2000
h2000_summary_final_test <- h2000_summary_final %>% 
   filter(BMI < 50 & Ferritin < 400)
...
test_data_pre_women <- test_data_pre_women %>% 
  dplyr::select(ID, Group, Sex, Age, donation_count, last_donation, Ferritin, Hb, BMI, Menstruation, Smoking, RedMeat_n, iron_complience, PreviousChildbirth, CRP, Cohort, donation_count_2, log_ferritin, log_last_donation, log_CRP, iron_deficiency) %>%
...
ggpairs(test_data_pre_women, ggplot2::aes(colour = Cohort, alpha = 0.5),
        columns = c("log_ferritin", "iron_deficiency", "PreviousChildbirth",  "RedMeat_n", "Smoking", "iron_complience", "BMI", "Menstruation"),
         lower = list(continuous = wrap("points", alpha = 0.3,size=0.1),
                      combo = wrap("facethist", binwidth = 0.5)),
        progress = FALSE) + 
ggtitle("Premenopausal women")
