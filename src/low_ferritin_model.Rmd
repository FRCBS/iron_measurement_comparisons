...
There are `r cohort_summary_name %>% filter(Sex == "Women" & is.na(Menstruation)) %>%  nrow()` women with no  answer to the question regarding their menstrual status. Of these, `r cohort_summary_name %>% filter(Sex == "Women" & is.na(Menstruation) & age > 55) %>%  nrow()` are older than 55. We impute these to no period so they can be included in the post-menopausal women. 
...
blood_data_summary_final <- blood_data_summary_final %>% 
  dplyr::select(ID, Group, Sex, Age, TwoYearsFromStartCount_FB, DaysToPreviousFB,
                Ferritin, Hb, BMI, Menstruation, Smoking, RedMeat, RedMeat_n, HistoryOfIronSupplements, GivenIronSupplements, IronComplience,
                IronComplienceNumeric, PreviousChildbirth, CurrentPregnancy, CRP, Cohort) %>% 
...
regression_cohorts <- summary_all_cohorts %>% 
    rename(donation_count = TwoYearsFromStartCount_FB,
     ...
     mutate(don_ct_2 = donation_count^2,
...
# manually entered N/A:s, is this why this part doesn't work?   
  
test_data_pre_women <- regression_cohorts %>% 
  filter(Group == "Pre_menopause_women") %>% 
  dplyr::select(ID, log_ferritin, Age, log_CRP, BMI, donation_count, log_last_donation,
                Smoking, PreviousChildbirth, GivenIronSupplements) %>%
  mutate(Age = Age / 5, 
         ...
         GivenIronSupplements = scale(GivenIronSupplements, scale = FALSE)[,1],
