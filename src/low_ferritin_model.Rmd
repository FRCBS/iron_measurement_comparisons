...
```{r}
#### FinDonor
nb_removed_BMI_blood <- findonor_nb_women_no_menstruation_response +
...
#### FinRisk97
nb_removed_BMI_fr97 <- finrisk_nb_women_no_menstruation_response +
...
#### Health2000
nb_removed_BMI_h2000 <- h2000_nb_women_no_menstruation_response +
...
# Select only the variables that are used in the analysis. 
# Assign smoking to "yes" if the study participant reported daily smoking and "no" if they reported no smoking or smoking sometimes +
# change the order of levels for smoking and pregnancy (no<yes)
...
We remove `r summary_all_cohorts %>% filter(is.na(DaysToPreviousFB)) %>% nrow()`, that is donors who have not donated previously (they are missing the nb of days since last FB donation variable)

```{r}
new_donors_data <- summary_all_cohorts %>% 
  filter(is.na(DaysToPreviousFB)) %>% 
    mutate(group = case_when(group == "Pre_menopause_women" ~ "Pre-menopausal women",
                   group == "Post_menopause_women" ~ "Post-menopausal women",
                   group == "Men" ~ "Men" ,
...
We remove `r summary_all_cohorts %>% filter(is.na(BMI)) %>% nrow()` participants for whom we do not have the BMI data:
