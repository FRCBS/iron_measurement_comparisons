...
fr1997_remove_na <- fr1997 %>%
  filter(!is.na(FERRITIN)) %>%
  group_by(RELEASE_ID) %>%
  dplyr::select(IKA, FERRITIN, CRP, RELEASE_ID, cohort) %>%
  ungroup()
...
## remove n/a:s for ferritin and Hb
  h2000_remove_na <- h2000 %>%
    ...
    group_by(RELEASE_ID) %>%
    dplyr::select(IKA2, FERRITIINI, CRP, RELEASE_ID, cohort) %>%
    ungroup()
...
FINDONOR:
There are `r blood_data_summary %>% filter(sex == "Women" & is.na(QR79)) %>%  nrow()` women with no  answer to the question regarding their menstrual status (do you still have regular periods?). Of these, `r blood_data_summary %>% filter(sex == "Women" & is.na(QR79) & age > 65) %>%  nrow()` are older than 65. We impute these to  no period so they can be included in the post-menopausal women. Other women are all under 55, thus their menstrual status cannot be ascertained. 

FINRISK97:
There are `r fr1997 %>% filter(SUKUP == "Women" & is.na(K129)) %>%  nrow()` women with no  answer to the question regarding their menstrual status (do you still menstruate?). Of these, `r blood_data_summary %>% filter(sex == "Women" & is.na(QR79) & age > 65) %>%  nrow()` are older than 65. We impute these to  no period so they can be included in the post-menopausal women. Other women are all under 55, thus their menstrual status cannot be ascertained. 


HEALTH2000:
BA3 = periods (1=reg, 2=irreg, 3=no)
...
blood_data_summary <- blood_data_summary %>% 
  mutate(QR79 = ifelse(donor %in% older_findonor_donors, "no_period", as.character(QR79))) 

#FINRISK97
older_fr1997 <- fr1997_summary %>% filter(sex == "Women" & is.na(K129)) %>%  nrow()

fr1997_summary <- fr1997_summary %>% 
  mutate(K129 = ifelse(RELEASE_ID %in% older_fr1997, "3", as.character(K129))) 

#HEALTH2000
older_h2000 <- h2000_summary %>% filter(sex == "Women" & is.na(BA3)) %>%  nrow()

h2000_summary <- h2000_summary %>% 
  mutate(BA3 = ifelse(RELEASE_ID %in% older_h2000, "3", as.character(BA3))) 
...
Female study participants with no menstruation response after imputation are removed: (FinDonor: `r blood_data_summary %>% filter(sex == "Women" & is.na(QR79)) %>% nrow()` donors.)


```{r}
#FINDONOR
findonor_nb_women_no_menstruation_response <- blood_data_summary %>% filter(sex == "Women" & is.na(QR79)) %>% nrow()
...
#FINRISK97
finrisk_nb_women_no_menstruation_response <- fr1997 %>% filter(SUKUP == "Women" & is.na(K129)) %>% nrow()


# remove female FinRisk97 participants with no menstruation response
fr1997_summary_final <- fr1997 %>% 
  mutate(mens_ok = case_when(
    SUKUP == "Men" ~ "Men", 
    SUKUP == "Women" & !is.na(K129)  ~ "Women",
    TRUE ~ "NA")) %>% 
  filter(mens_ok != "NA") 
...
#   mutate(group = case_when(
#     SUKUP == "Men" ~ "Men", 
#     SUKUP == "Women" & (K129 == "no_period" & IKA > 45) ~ "Post_menopause_women",
#     SUKUP == "Women" & (K129 == "no_period" & IKA <= 45) ~ "Women_pre_menop_no_mens",
#     SUKUP == "Women" & (K129 == "irregular_period" | K129 == "regular_period") ~ "Pre_menopause_women",
#     TRUE ~ "NA")) %>% 
#   group_by(group) %>% 
#   filter(group ==  "Women_pre_menop_no_mens") %>% 
#   nrow()
...
fr1997_summary_final <- fr1997_summary_final %>% 
  mutate(group = case_when(
    SUKUP == "Men" ~ "Men", 
    SUKUP == "Women" & (K129 == "3" & IKA > 45) ~ "Post_menopause_women",
    SUKUP == "Women" & (K129 == "3" & IKA <= 45) ~ "Women_pre_menop_no_mens",
    SUKUP == "Women" & (K129 == "1" | K129 == "regular_period") ~ "Pre_menopause_women_regular",
    SUKUP == "Women" & (K129 == "2" | K129 == "irregular_period") ~ "Pre_menopause_women_irregular",
    TRUE ~ "NA")) %>% 
  filter(group != "Women_pre_menop_no_mens" & group != "NA") %>% 
  droplevels() %>% 
  mutate(group = ordered(group, levels =  c("Pre_menopause_women", "Post_menopause_women", "Men"))) 
...
# We then  assign a donation interval for 3-6 years for the general population cohorts
...
# We rename the variables in the different data sets to maintain cohesion 

## Participant ID 
names(blood_data_summary_final)[names(blood_data_summary_final) == "donor"] <- "ID"
names(fr1997_summary_final)[names(fr1997_summary_final) == "RELEASE_ID"] <- "ID"
## Sex
names(fr1997_summary_final)[names(fr1997_summary_final) == "SUKUP"] <- "sex"
## Age
names(fr1997_summary_final)[names(fr1997_summary_final) == "IKA"] <- "age"
## Hemoglobin
names(fr1997_summary_final)[names(fr1997_summary_final) == "HGB"] <- "Hb"
## Ferritin
names(fr1997_summary_final)[names(fr1997_summary_final) == "FERRITIN"] <- "Ferritin"
...
summary_all_cohorts <- rbind(blood_data_summary_final, fr1997_summary_final)
...
* CRP or sTfR: `r blood_data_summary_final %>% filter(is.na(CRP) | is.na(TransferrinR)) %>% nrow()` donors - !!!!!! skip sTfR  part, we aren't using it. CRP? !!!!!
