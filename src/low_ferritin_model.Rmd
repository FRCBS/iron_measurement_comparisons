...
CHECK THAT THIS ACTUALLY ONLY REMOVES PREGNANT WOMEN!!!!!!
...
We use a linear scale from 1 to 4 for red meat consumption and mapped the different responses to correspond to the four options for the FinDonor and FinRisk97 cohorts. Data on red meat consumption was not avaliable in the Health2000 cohort.  
...
* red meat:
    ...
    * FinRisk97
    ...
    NOTE: DIFFERENT CODING IN FINDONOR REPORT, WHY IS THIS DIFFERENT? 1=can't say, 2=none, 3=once a week or less, 4=1-3 times a week, 
    5=4-6 times a week, 6=once a day

```{r}
# Recode red meat consumption to linear variable 

# FinDonor
meat_blood <- blood_data_summary_final %>% 
...
blood_data_summary_final <- blood_data_summary_final %>% 
  full_join(meat_blood, by = "ID") 

# FinRisk
## DOUBLE CHECK THAT THE CODING FOR RED MEAT CONSUMPTION ACTUALLY MATCHES THE ONE I'M USING!!!!!!
meat_fr1997 <- fr1997_summary_final %>% 
  dplyr::select(ID, RedMeat) %>% 
  gather(key = key, value =  value, -ID) %>% 
  mutate(linear_value = case_when(value == "1" ~ 1,
    value == "2" ~ 2,
    value %in% c("3" , "4", "5") ~ 3,
    TRUE   ~ 4 )) %>% # "6" | "7"
  dplyr::select(-value) %>% 
...
fr1997_summary_final <- fr1997_summary_final %>% 
  full_join(meat_fr1997, by = "ID") 

# this part of the code creates a new variable, RedMeat_n. As we don't have red meat data from the Health2000 cohort, we need to make a column, name it RedMeat_n and input the observations as n/a.
...
```{r}
# Recode food and non-alcoholic beverages THIS PART NEEDS TO BE RECODED?
