...
##removed most factors and all renaming/mutating just to try out this feature, original script for reference:
blood_data_summary_final <- blood_data_summary_final %>%
  dplyr::select(ID, group, sex, age, TwoYearsFromStartCount_FB, DaysToPreviousFB, Hb_v,
       Ferritin, CRP, DaysToPreviousFB, QR40, iron_comp_c, BMI, QR83, iron_supp, QR54) %>%
  rename(red_meat = QR40,
         pregnancy = QR83) %>%
    mutate(smoking = ifelse(QR54 == "no", "no", "yes"),
         smoking = ordered(smoking, levels = c( "no",  "yes")),
         pregnancy = ordered(pregnancy, levels = c( "no",  "yes"))) %>%
  dplyr::select(-QR54)# Rename variables to maintain cohesion between the cohorts
## Participant ID 
# names(blood_data_summary)[names(blood_data_summary) == "donor"] <- "ID"
# names(fr1997_summary)[names(fr1997_summary) == "RELEASE_ID"] <- "ID"
# names(h2000_summary)[names(h2000_summary) == "RELEASE_ID"] <- "ID"
...
# names(h2000_summary)[names(h2000_summary) == "IKA2"] <- "Age"
# ## Hemoglobin
# names(blood_data_summary)[names(h2000_summary) == "Hb_v"] <- "Hb"
# names(fr1997_summary)[names(fr1997_summary) == "HGB"] <- "Hb"
# names(h2000_summary)[names(h2000_summary) == "B_Hb"] <- "Hb"
# ## Ferritin
# names(fr1997_summary)[names(fr1997_summary) == "FERRITIN"] <- "Ferritin"
